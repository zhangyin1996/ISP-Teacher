...
def Low_Illumination_Degrading(img, safe_invert=False):
    ...
    red_gain = random.uniform(transformation_para['red_range'][0], transformation_para['red_range'][1])
    blue_gain = random.uniform(transformation_para['blue_range'][0], transformation_para['blue_range'][1])
    ...
    bits = random.choice(transformation_para['quantisation'])
    ...
    para_gt = torch.FloatTensor([darkness, 1.0 / gamma, 1.0 / red_gain, 1.0 / blue_gain]).to \
        (torch.device(device))
